...
def backup():
    announce_in_server(
        "Beginning server backup, server might have reduced performance during this time"
    )
    ...
    # COMPRESS!!!!!
    announce_in_server("(starting backup compression on disk)")
    ...
    announce_in_server("(backup done; full server performance capability restored)")
